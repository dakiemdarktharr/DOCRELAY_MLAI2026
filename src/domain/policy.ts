...
function isMissing(field: string, value?: string) {
  ...
  if (
    field === "reason" &&
    (/^(debug|test|needed|can gap|khac phuc|support)$/i.test(normalize(value)) ||
      value.trim().length < 8)
  )
    return true;
  if (
...
function evaluateSinglePolicy(
      ...
      assignedTeam:
        bucket === "SECURITY_RISK"
          ...
          : databaseAuthority
            ...
            : base.assignedTeam,
      nextStep:
        "Reviewer kiểm tra evidence, approval và phạm vi; hệ thống không thực hiện thay đổi thật.",
      questions: [...new Set(matched.map((rule) => rule.question))],
    };
  }
  ...
  if (missing.length) {
    ...
    const questions = uniqueMissing.map(questionForField);
    return {
      ...base,
  ...
  if (guide && ["GUIDANCE", "SAFE_DIAGNOSTIC"].includes(request.requestKind)) {
    const medium = ["vpn", "wifi", "software", "account"].includes(
      guide.topic,
    );
    return {
      ...base,
  ...
  const allowed =
    ...
    (request.serviceGroup === "GIT_PERMISSION" &&
      ["GIT_READ_ACCESS", "GIT_WRITE_ACCESS"].includes(
        request.intentLabel,
      ) &&
...
export function evaluatePolicy(
  ...
  const synchronizeQuestions = (decision: Decision): Decision => ({
    ...decision,
    targetedQuestions: decision.questions,
  });
  ...
  return synchronizeQuestions({
    ...
    questions: [...new Set(all.flatMap((decision) => decision.questions))],
