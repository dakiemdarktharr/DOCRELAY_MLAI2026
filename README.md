...
5. Mở /verify, chạy pack submission-4 rồi de-a-v3 để xem ba case auto và hai case escalate.
...
| Pack | Mục đích |
| --- | --- |
| submission-4 | Bộ demo ngắn có routine, thiếu thông tin và escalation |
| de-a-v3 | **5 cases: 3 auto / 2 escalate**, phù hợp đường Verify của Đề A |
| support-v3, official, synthetic, extended, crossFunction | Regression và kiểm tra biên trong mlai26_new/data/verify/ |
...
## Vercel readiness — kiểm tra chỉ đọc

Tình trạng này là **read-only audit**, không phải thông báo đã deploy:

- Repo là Next.js app có vercel.json; npm ci và npm run build chạy được trên main.
- main tại thời điểm audit: 825df2a120ba99e28fde08bb8460dadecbd01421.
- Live health endpoint [api/support/health](https://vng-support.vercel.app/api/support/health) phản hồi status ok, storage MongoDB durable và policy support-guidance-v5.2.
- Live sourceRevision là 01a20a7128b58a0e09e8e6f6ed1fe10a1f4a85a3, khác với main; host đang chạy revision cũ hơn main.
- Chưa tạo Vercel project, chưa import repo, chưa đổi Production Branch và chưa deploy. Dashboard branch binding chưa được xác minh.

> Muốn host chạy đúng main, đặt **Production Branch = main** trong project Vercel hiện có, giữ root directory là repo root, rồi deploy theo quy trình của chủ project. Sau đó đối chiếu sourceRevision với SHA của main; health ok riêng lẻ chưa đủ.

Xem checklist trong [docs/VERCEL-READINESS.md](docs/VERCEL-READINESS.md).
