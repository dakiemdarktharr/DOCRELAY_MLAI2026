# Historical snapshot — superseded by docs/RELEASE-MATRIX.md

Snapshot retained verbatim below from main 20727b5. Relative links below are historical and resolve from the repository root. Do not use these commands or old current-state claims as current guidance.
...
- Optional `AI_CONVERSATION_MODEL` and `AI_WEB_MODEL` otherwise inherit `AI_MODEL`. Production explicitly uses `AI_WEB_MODEL=gpt-4.1`: the inherited model returned HTTP 400 for the hosted tools configuration, while gpt-4.1 was verified with real citations. `AI_WEB_SEARCH=true` enables Responses hosted web search for public company/GPU questions. Only server-owned queries and approved domains are searched. No shell, MCP, function or database tools are exposed to the LLM. `v3_support_web_cache` expires after 24 hours and is distinct from reviewed knowledge.
