> Current release work: main `7909f0c` has been integrated with policy v5.3, judge onboarding and QR access. The user authorized Vercel deployment after checks; see [release readiness](VERCEL-READINESS.md). The older snapshot below is historical and does not establish the current deployed revision. The two-package requirement no longer applies.
